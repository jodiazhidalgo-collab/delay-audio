...
class PreviewEditTests(unittest.TestCase):
    def test_preview_manifest_keeps_core_mapping_profile_tempo_and_hint(self):
        ...
        plan = {
            ...
            "spanish_clip_start_sec": 3105.6,
            "spanish_source_duration_sec": 28.8,
        ...
        try:
            ...
            self.assertEqual(result["spanish_clip_start_sec"], 3105.6)
            self.assertEqual(generate.call_count, 2)
            self.assertEqual(generate.call_args_list[0].args[3:6], (3237.0, 30.0, 1.0))
            self.assertEqual(generate.call_args_list[1].args[3:6], (3105.6, 30.0, 0.96))
    ...
    def test_preview_clip_applies_nonzero_seek_and_provisional_tempo(self):
        ...
        try:
            with (
                ...
                routes.generar_preview_clip("esp.mkv", target, "Audio Español", 3105.6, 30.0, 0.96)
            command = captured["command"]
            self.assertEqual(command[command.index("-ss") + 1], "3105.600000")
            self.assertIn("setpts=(PTS-STARTPTS)/0.960000000000", command[command.index("-vf") + 1])
            self.assertIn("28.800000", command)
        finally:
            shutil.rmtree(runtime_root, ignore_errors=True)
